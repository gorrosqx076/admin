...
// --- LÓGICA DEL MODO MANTENIMIENTO (ADMIN) ---
let estadoMantenimiento = false;

// Leer si la página está apagada o prendida
db.ref('Configuracion/Mantenimiento/activo').on('value', (snap) => {
    estadoMantenimiento = snap.val() || false;
    const btn = document.getElementById('btn-mantenimiento');
    if(btn) {
        if(estadoMantenimiento) {
            btn.innerText = "🔴 DESACTIVAR MANTENIMIENTO (Página Apagada)";
            btn.style.background = "#ff4757"; // Rojo porque está apagada
        } else {
            btn.innerText = "🟢 Activar Modo Mantenimiento";
            btn.style.background = "#2ed573"; // Verde porque está jalando normal
        }
    }
});

// Función para prender/apagar
window.toggleMantenimiento = function() {
    const nuevoEstado = !estadoMantenimiento;
    if(nuevoEstado) {
        if(!confirm("⚠️ ¿Seguro que quieres APAGAR la página? Los clientes verán la pantalla rosa de mantenimiento.")) return;
    } else {
        if(!confirm("✅ ¿Seguro que quieres volver a PRENDER la página para los clientes?")) return;
    }
    db.ref('Configuracion/Mantenimiento/activo').set(nuevoEstado);
}
